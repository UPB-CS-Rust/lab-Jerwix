...
fn get_username() -> Result<String, MyError> {
    ...
    let ceva2 = io::stdin().lock().read_line(&mut input);
    match ceva2 {
        Ok(_) => {},
        Err(e) => return Err(MyError::IOError(e)),
    }
...
fn main() {
    ...
    match res {
        Ok(name) => {println!("Hello {name}!"); return},
        Err(MyError::InvalidName) => println!("Try another name"),
        Err(MyError::IOError(e)) =>  {eprintln!("{e}"); return},
    }
    }
}
